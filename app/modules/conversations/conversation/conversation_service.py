...
from .structured_repo_analysis import run_structured_repo_analysis
    ...
    async def _generate_and_stream_ai_response(
        ...
        try:
            ...
            if type == "CUSTOM_AGENT":
                # 1) Decidi quando vuoi JSON strict
                structured_json_mode = "STRICT OUTPUT RULES" in query  # oppure un flag nella request

                if structured_json_mode:
                    # 2) NON streammare dal custom agent, genera output strutturato
                    obj = await run_structured_repo_analysis(query)
                    json_text = obj.model_dump_json(by_alias=True, exclude_none=False)

                    # 3) Stream controllato: invii solo JSON valido a chunk
                    chunk_size = 1024
                    citations: list[str] = []
                    tool_calls: list[str] = []
                ...
                # ---- default: comportamento attuale streaming del custom agent ----
                res = await self.agent_service.custom_agent_service.execute_agent_runtime(
                    user_id,
                    ChatContext(
                ...
                async for chunk in res:
                    ...
                    yield ChatMessageResponse(
                        ...
                        citations=chunk.citations,
                        tool_calls=[tool_call.model_dump_json() for tool_call in chunk.tool_calls],
                    )
                self.history_manager.flush_message_buffer(conversation_id, MessageType.AI_GENERATED)
            else:
            ...
            structured_json_mode = "STRICT OUTPUT RULES" in query
            if structured_json_mode:
                obj = await run_structured_repo_analysis(query)
